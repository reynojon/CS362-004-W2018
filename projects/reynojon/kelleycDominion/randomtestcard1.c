 ...
 *
 * Tests the playCouncilRoom function
 */
...
#define DEBUGGING 0
#define NUM_TESTS 9 //Number of times assert functions are called per iteration
...
void assertCouncilRoomDraw(int firstCard, int secondCard, int thirdCard, int fourthCard, int firstDraw, int secondDraw, int thirdDraw, 
                           int fourthDraw, int *arr, int *testFailed, int *failArray, int failIndex);
...
void assertOtherPlayerDraw(int *handSizes, int *deckSizes, int *discardSizes, struct gameState *state, int *testFailed,
	int *failArray, int failIndex, int numPlayers);
void assertPlayed(int expectedPlayedCount, struct gameState *state, int *testFailed, int *failArray, int failIndex);
void assertBuys(int expectedBuys, struct gameState *state, int *testFailed, int *failArray, int failIndex);
...
int main()
{
  ...
  numPlayers = (rand() % (MAX_PLAYERS - 1)) + 2; //At least 2 players
    
  int k[10] = {adventurer, gardens, embargo, village, minion, mine, cutpurse,
           sea_hag, tribute, smithy}; //These do no matter, but values are required to use initializeGame()
  ...
  int firstDraw, secondDraw, thirdDraw, fourthDraw, randHandPos;
  int failCount[NUM_TESTS];  //Tracks the number of tests failed
  int testFailed = 0;   //Used as a boolean, must be either 0 or 1;
  int initialCardCount, initialScore, initialDeckCount, initialDiscardCount, initialHandCount, initialBuy;
  ...
  for(i = 0; i < NUM_ITERATIONS; i++)
   {
      ...
      //Put an Council Room card in hand at a random position
      randHandPos = rand() % G.handCount[G.whoseTurn];
      G.hand[G.whoseTurn][randHandPos] = council_room;
    
      //Record values prior to playing Council Room card
      ...
      {
        playerHandSizes[j] = G.handCount[j];
        playerDeckSizes[j] = G.deckCount[j];
        playerDiscardSizes[j] = G.discardCount[j];
      }
      ...
      if(fourthCard == -1)
      {
        ...
        if(((firstCard == -1) && (G.discardCount[G.whoseTurn] < 3)) || ((secondCard == -1) && (G.discardCount[G.whoseTurn] < 2)) ||
	  ...
	  testFailed = 0;
      assertCouncilRoomDraw(firstCard, secondCard, thirdCard, fourthCard, firstDraw, secondDraw, thirdDraw, fourthDraw, 
			tempDeck, &testFailed, failCount, 0);
      ...
      {
		testFailed = 0;
        assertDeckSize(initialDeckCount - 4, &G, &testFailed, failCount, 3); //Verify cards in player's deck changed by correct amount (if not shuffled)
		testFailed = 0;
	  ...
	  testFailed = 0;
      assertHandSize(initialHandCount + 3, &G, &testFailed, failCount, 5);
    
      //Verify hand sizes of other players increased by 1
	  testFailed = 0;
      assertOtherPlayerDraw(playerHandSizes, playerDeckSizes, playerDiscardSizes, &G, &testFailed, failCount, 6, numPlayers);
    
      //Verify council_room card put on top of played pile and playedCardCount was incremented
	  testFailed = 0;
      assertPlayed(1, &G, &testFailed, failCount, 7);
	  
	  //Verify number of buys increased by 1
	  testFailed = 0;
      assertBuys(initialBuy + 1, &G, &testFailed, failCount, 8);
  ...
  printf("assertCouncilRoomDraw failed %d times.\n", failCount[0]);
  ...
  printf("assertOtherPlayerDraw failed %d times.\n", failCount[6]);
  printf("assertPlayed failed %d times.\n", failCount[7]);
  printf("assertBuys failed %d times.\n\n\n", failCount[8]);
                           ...
                           int fourthDraw, int *arr, int *testFailed, int *failArray, int failIndex)
{
  ...
  else if (firstCard != firstDraw)
  {
	if(DEBUGGING)
	{
		printf("-----FAILURE: First card drawn from Council Room was not first card in the deck!\n");
	}
    *testFailed = 1;
  }
  ...
  else if (secondCard != secondDraw)
  {
	if(DEBUGGING)
	{
		printf("-----FAILURE: Second card drawn from Council Room was not second card in the deck!\n");
	}
    *testFailed = 1;
  }
  ...
  else if (thirdCard != thirdDraw)
  {
	if(DEBUGGING)
	{
		printf("-----FAILURE: Third card drawn from Council Room was not third card in the deck!\n");
	}
    *testFailed = 1;
  }
  ...
  else if (fourthCard != fourthDraw)
  {
	if(DEBUGGING)
	{
		printf("-----FAILURE: Fourth card drawn from Council Room was not third card in the deck!\n");
	}
    *testFailed = 1;
  }
  ...
  for(i = 0; i < numPlayers; i++)
  {
    if(i != state->whoseTurn)
    {
      ...
      if((deckSizes[i] - 1) != state->deckCount[i])
      {
		if(DEBUGGING)
		{
			printf("-----FAILURE: Player Number %d deck size incorrect!\n", i);
		}
        *testFailed = 1;
      }
      if((discardSizes[i]) != state->discardCount[i])
      {
		if(DEBUGGING)
		{
			printf("-----FAILURE: Player Number %d discard size incorrect!\n", i);
		}
        *testFailed = 1;
      }
    }
...
//Checks if playedCardCount matches the expected parameter and that the Council Room card is on top of the played pile
//Prints a message if the check fails
// If DEBUGGING is 1, a pass message will be printed if the check passes
//If the check fails, testFailed is set to 1 (true)
// Does not change the game state
void  assertPlayed(int expectedPlayedCount, struct gameState *state, int *testFailed, int *failArray, int failIndex)
{  
  if((expectedPlayedCount != state->playedCardCount) || (state->playedCards[state->playedCardCount - 1] != council_room))
  {
    if(DEBUGGING)
	{
		printf("-----FAILURE: Council Room not properly added to played pile!\n");
	}
    *testFailed = 1;
  }  
  else if (DEBUGGING)
  {
    printf("-----PASS: Council Room correctly added to played pile!\n");
  }
...
void assertBuys(int expectedBuys, struct gameState *state, int *testFailed, int *failArray, int failIndex)
{  
  if(expectedBuys != state->numBuys)
  {
    if(DEBUGGING)
	{
		printf("-----FAILURE: Council Room did not increase number of Buys by 1!\n");
	}
    *testFailed = 1;
  }  
  else if (DEBUGGING)
  {
    printf("-----PASS: Council Room correctly increased number of buys!\n");
  }
